...
module.exports = {
  ...
  'db:migrate:schema:add_timestamps': {
    descriptions: {
      ...
      .then(function (migrator) {
        ...
        .then(function (items) {
          if (items) {
            ...
            console.log('MetaTable already had timestamps added.');
          }
...
function ensureCurrentMetaSchema (migrator) {
  ...
  .then(function (table) {
    ...
    if ((columns.length === 1) && (columns[0] === columnName)) {
      ...
      return;
    } else if ((columns.length === 1) && (columns[0] === columnName)) {
      console.error(
        'Added timestamps option without migrating. Please run ' +
        '"sequelize db:migrate:schema:add_timestamps" if you truly wish to ' +
        'add timestamps to your meta options.'
      );
      process.exit(1);
    } else {
...
function addTimestampsToSchema (migrator) {
  ...
  return ensureMetaTable(queryInterface, tableName)
  .then(function (table) {
    if (Object.keys(table).indexOf('createdAt') !== -1) {
      return;
    }
